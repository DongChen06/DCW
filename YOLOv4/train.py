...
if __name__ == '__main__':
    ...
    if opt.resume:  # resume an interrupted run
        ...
        opt.save_dir = increment_path(opt.project + '/' + str(opt.weights[:-3]) + '_' + str(opt.device), exist_ok=opt.exist_ok | opt.evolve)  # increment run
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {'lr0': (1, 1e-5, 1e-1),  # initial learning rate (SGD=1E-2, Adam=1E-3)
                ...
                'anchors': (2, 2.0, 10.0),  # anchors per yolov4 grid (0 to ignore)
